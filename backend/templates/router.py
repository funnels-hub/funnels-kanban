...
async def list_templates_endpoint():
    try:
        return list_templates_main()
    except ValueError as e:
        _handle(e)


@router.post("")
async def create_template_endpoint(body: TemplateCreate):
    try:
        source_date = body.source_date.isoformat() if body.source_date else None
        return create_template_main(body.name, source_date)
    except ValueError as e:
        _handle(e)


@router.get("/{tpl_id}")
async def get_template_endpoint(tpl_id: str):
    try:
        return get_template_main(tpl_id)
    except ValueError as e:
        _handle(e)


@router.patch("/{tpl_id}")
async def update_template_endpoint(tpl_id: str, body: TemplateUpdate):
    try:
...
async def delete_template_endpoint(tpl_id: str):
    try:
        return delete_template_main(tpl_id)
    except ValueError as e:
        _handle(e)


@router.post("/{tpl_id}/duplicate")
async def duplicate_template_endpoint(tpl_id: str):
    try:
        return duplicate_template_main(tpl_id)
    except ValueError as e:
        _handle(e)
