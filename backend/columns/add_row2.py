...
def main(date: str, row1_id: str, label: str, position: int | None = None) -> dict:
    conn = get_db_connection()
    try:
        ensure_columns_for_date(conn, date)
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT 1 FROM column_row1 WHERE date = %s AND id = %s",
                (date, row1_id),
            )
            if not cur.fetchone():
            ...
            if position is None:
                cur.execute(
                    ...
                    "FROM column_row2 WHERE date = %s AND row1_id = %s",
                    (date, row1_id),
                )
            ...
            cur.execute(
                ...
                (new_id, date, row1_id, label, position, False),
            )
...
if __name__ == "__main__":
    ...
    result = main(args.date, args.row1_id, args.label, args.position)
